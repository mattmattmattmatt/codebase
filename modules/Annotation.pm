...
sub new {
    ...
    my $config = defined $args{-config}?$args{-config}:modules::Pipeline->get_report_conf();
    ...
    if ($organism eq 'human') {
		@annotation_headers = split(",",$config->read($mutant_type,'common','human_file_annotations'));
	} elsif ($organism eq 'mouse') {
		@annotation_headers = split(",",$config->read('snv','common','mouse_file_annotations'));
	} else {
		modules::Exception->throw("ERROR: -organism must be mouse or human");
	}
...
sub _create_map {;
    ...
    while (<ANNO>) {
		...
		if (@fields != @annotation_headers+1) {
			print Dumper \@annotation_headers;
			print Dumper \@fields;
...
sub get_gene_from_name {
	my $self = shift;
	my %args = @_;

    my @required_args = ('-gene_name');
    ...
    if (!exists $self->{exon_map}{$gene_name}) {
    	modules::Exception->throw("ERROR: Can't find entry for gene $gene_name");
    } 
    
    my @annotation_headers = @{$self->{annotation_headers}};
    $gene_info{ensembl}{$self->{exon_map}{$gene_name}{ensembl}}++;
    for my $annotation_header ( @annotation_headers ) {
		$gene_info{$annotation_header}{$self->{exon_map}{$gene_name}{$annotation_header}}++;
	}
    return \%gene_info;
}
