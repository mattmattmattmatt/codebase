...
sub insert 
{
	...
	my $field_number = @{$ra_row_data};
	print STDERR "Insert $field_number entries into table $table_name\n";
...
sub dbh {
    ...
    if ($args{-db} eq 'MelDB') {
    	return DBI->connect(modules::Adaptors::VariantDB->getConfig) || modules::Exception->throw("Unable to connect to database: $DBI::errstr");
    } else {
