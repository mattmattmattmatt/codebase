...
modules::Adaptors::Variant->columns('All' => qw/id chr start_coord end_coord var_type var_score clr_score affected_bases read_depth ref_base_freq var_base_freq normal_base_string tumour_base_string median_quality_score variant_class run_id/);
...
modules::Adaptors::Variant->set_sql('region' => 'SELECT * FROM variants WHERE variants.run_id = ? AND chr = ? AND start_coord >= ? AND end_coord <= ? ORDER BY chr, start_coord');
