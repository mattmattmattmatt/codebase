...
run_pipeline_steps.pl -xml xml_config_file -no_run do_everything_except_the_step -no_cleanup_db no_cleanup_db(except pipeline_steps_run AND syscalls) -no_cleanup_files no_cleanup_files -latest_sample get_the_latest_run_for_this_sample -create_run just_create_run_and_quit -resume resume_at_specific_step(stepname_from_xml) -last_step last_step_to_run -clean_only only_clean_steps [options]
...
if ($step_resume) {
	...
	if ($run_config->exists('steps', 'step', $steps[0],'first_step')) {
	    ...
	    @lanes_objs = modules::Adaptors::Lane->search(sample_id=>$sample_obj->id);
...
sub Initialize
{
	...
	my $xml_file_dest = $confdir.'/'.$rundir_name.'.conf.xml';
	...
	$log->append("BINARIES:");
	my $bin_struct = $pipe_config->read($source_type,'binaries');
...
sub Run_Pipeline_Steps {
	...
	for (my $i = 0; $i < scalar @steps; $i++) {
		...
		my $outsv = $workdir . '/sv/' . $rundir_name;
	    ...
	    if ($run_config->read('steps', 'step', $step, 'by_chr')) {
	    	my @chromosomes = split(" ",$pipe_config->read($source_type,'annotation_version','chr'));
			...
			if ($run_config->exists('steps', 'step',$step,'release_file')) {
				...
				if ($release_suffix =~ /bam/) {
					$file_type = 'bam';
					$total_lanes = $sample_obj->total_lanes;
				} elsif ($release_suffix =~ /SVCALLER/) {
					$file_type = 'summary';
					$total_lanes = $sample_obj->total_lanes;
					my @sv_callers = split(",",$pipe_config->read('common','sv_callers'));
					
					for my $sv_caller (@sv_callers) {
						my $release_file_name = $rundir_name.$release_suffix;
						
						(my $release_file_name_final = $release_file_name) =~ s/SVCALLER/$sv_caller/;
						my %release_file = (
	    							file_name=>$release_file_name_final,
	    							file_type=>$file_type,
					...
					#Create combined file as well
					my $release_file_name = $rundir_name.$release_suffix;
						
					(my $release_file_name_final = $release_file_name) =~ s/SVCALLER/combined/;
					my %release_file = (
	    							file_name=>$release_file_name_final,
	    							file_type=>$file_type,
	    							pipeline_steps_run_id=>$pipeline_step_run_id,
					...
					if ($tumour_flag) {
						...
						(my $sample_name_normal = $sample_name) =~ s/tumour[0-9]/normal1/;
						#my ($source_group_obj)  = modules::Adaptors::Source_Group->search('id'=>$sample_obj->source_group_id);
						my ($normal_sample_obj) = modules::Adaptors::Sample->search('source_group_id'=>$sample_obj->source_group_id,'sample_name'=>$sample_name_normal);
						if (!defined $normal_sample_obj) {
							modules::Exception->throw("ERROR: Cannot retrieve normal sample for sample $sample_name");
						}
						$total_lanes = $sample_obj->total_lanes + $normal_sample_obj->total_lanes;
					} else {
						$total_lanes =  $sample_obj->total_lanes;
					}
	    		...
	    		modules::Adaptors::Release_File->insert(\%release_file) unless $release_suffix =~ /SVCALLER/;
	    	}
	    }
...
sub CleanUp_Step {
	...
	my $sv_files_to_remove = $svdir .'/*.'. $stepname .'.*';
	...
	my $conf_files_to_remove = $confdir.'/*.' . $stepname . '.*';
	
	if ($cleanup_files) {
		#Don't use sys_call as we don't want to die if there are no files to remove
		system("rm $sv_files_to_remove 2>/dev/null");
		...
		system("rm $conf_files_to_remove 2>/dev/null");
	}
...
sub Create_Dirs {
    ...
    my @subdirs = split(",",$pipe_config->read('common','run_subdirs'));
...
sub Set_Paths
{
	...
	$workdir = join('/',$rundir_base,$source_name,$sample_name.'_runs',$rundir_name);
