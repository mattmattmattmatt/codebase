...
get_bam_stats.pl -bam bamfile -exon exonfile_if_want_exon_stats -output output_file -no_prod -samtoolsbin samtools_binary [options]
...
unless ($OPT{no_prod}) {
	...
	if (!defined $sample_obj) {
		modules::Exception->throw("ERROR: Can't find sample $sample_name");
	}
	
	$seq_type = $sample_obj->sequence_type;
	$samtools_bin = $pipe_config->read($source_type,'binaries','samtools','binary');
} else {
	if (defined $OPT{samtoolsbin}) {
		$samtools_bin = $OPT{samtoolsbin};
	} else {
...
sub Read_Stats {
	...
	#Do this for genomes now as well -> for NATA reports
	if ($OPT{exon}) {
		my $exon_file = $OPT{exon};
		if (!-e $exon_file) {
			modules::Exception->throw("ERROR: Cannot open exon file $OPT{exon}");
		}
		open(EXON,"$exon_file") || die "Can't open file $exon_file\n";
