...
pod2usage(1) if ($OPT{help} || !$OPT{sample_name} || !$OPT{output}  || !$OPT{ref} || !$OPT{bam} || !$OPT{outvcf});
...
if ($tumour_flag) {
	...
	$normal_bam = $rundir . '/' . $source_name . '/bam_links/'. $bam_norm;
	if (!-e $normal_bam ) {
...
my $outdir = $cluster_config->read($source_type,'base_directories','base_run_directory');

my $vcf_dir = $outdir.'/'.$source_name.'/'.$source_group_name.'_snvcalls';
if (!-d $vcf_dir) {
...
$vcf->parse_vcf(-vcf_file => $all_vcf_file,-tumour_flag=>$tumour_flag);
...
while (<INDELPILEUP>) {
    $_ =~ s/^chr//;
	my @fields = split("\t");
	my $indel_start = $fields[1];
	if (exists $del_coords{"$fields[0]:$fields[1]"}) {
		$indel_start++;
	} 
...
unless ($OPT{no_cleanup_files}) {
	...
	my @delete_files = grep {/[sb]a[im]$/} readdir LANES;
	closedir LANES;
	...
	if ($tumour_flag) {
		...
		my $normal_lane_dir = $rundir . '/' . $source_name . '/' . $source_group_name . '_normal1_lanes';
