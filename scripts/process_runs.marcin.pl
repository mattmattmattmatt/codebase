...
####################################################################################################
#
print "\nRuns ready to submit cohort reports...\n";

#Retrieve all the sample groups
my @source_groups = modules::Adaptors::Source_Group->search_all();
for my $source_group_obj(@source_groups)
{
	my $source_group_not_ready = 0;
	
	my $sample_total_field = $source_group_obj->total_samples;
	my $source_group_name  = $source_group_obj->source_group_name;
	my $source_type        = modules::Pipeline::get_source_type(-source_group_name => $source_group_name);

	next if($source_type !~ /_related/);

	my $last_pipe_step     = modules::Pipeline::get_xml_step(-source_type => $source_type, -xml_name=>'last');
	
	#print "$source_group_name\t$source_type\t$last_pipe_step\n";
	...
	for my $sample_obj(@samples) 
	{
		...
		if(defined $run_obj) 
		{
	  	my $run_id = $run_obj->id;
	    my @last_step = modules::Adaptors::Pipeline_Step_Run->search(run_id => $run_id, pipeline_step_id => $step_ids{$last_pipe_step});
			if(!@last_step) #hasn't completed the latest step, meaning they can't run cohort summaries
			{
				$source_group_not_ready = 1;
				#my $last_step_run = modules::Pipeline::get_latest_step($run_id);
				#print STDERR "Sample $sample_name has only run up to step $last_step_run...\n";
			}	
	  }
	  else #has no runs! Too bad
	  {
	  	$source_group_not_ready = 1;
	  }
	}
	
	if(!$source_group_not_ready)
	{
		my @summary_groups = modules::Adaptors::Group_Summary->search(source_group_id=>$source_group_obj->id);
		if(!@summary_groups) 
		{
			my($source) = modules::Adaptors::Source->search(id => $source_group_obj->source_id);
			my $external_name = $source->external_source_name;
			my $qsub_dir  = $cluster_config->read($source_type, 'base_directories', 'base_qsub_directory') . '/'. $external_name .'/runs';
			my $qsub_file = "$external_name\_cohort.qsub";
	
			my $job_id = $cluster_obj->check_job_running(-sample_name => "$external_name\_cohort");
			if($job_id)
			{
				print STDERR "$external_name has already submitted job $job_id and is waiting for it to finish\n";
			}
			else
			{
				print STDERR "$external_name is ready to submit cohort report\n";
				if(-s "$qsub_dir/$qsub_file")
				{
					push @commands, "qsub $qsub_dir/$qsub_file";
				}
				else
				{
					print STDERR "WARNING: cohort report submission file '$qsub_dir/$qsub_file' can't be stated; report will not be submitted\n";
				}
			}
		}
	}
}#for my $source_group_obj(@source_groups)
#end of "Runs ready to submit cohort reports...\n";

if(@commands)
{
	if($OPT{user})
	{
		print STDERR "\nFor user $OPT{user} only; run the following commands:\n\n";
	}
	else
	{
...
for my $command ( @commands ) 
{
	...
	{
		$process = 0;
		modules::Exception->throw("ERROR: Couldn't get qsub file name from commad '$command'\n") if($command !~ /(\S+)$/);
		my $qfile = $1;
		my @fstat = stat($qfile);
		modules::Exception->throw("ERROR: Couldn't get user id for file '$qfile'\n") if(!@fstat);
		my @user = getpwuid($fstat[4]);
		modules::Exception->throw("ERROR: Couldn't get user name for file '$qfile'\n") if(!@user);
		$process = 1 if($OPT{user} eq $user[0]);
	}
	if($process)
	{
