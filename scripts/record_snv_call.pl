...
record_snv_call.pl -step_name step_name_to_record -runid runid -chr chr -vcf vcf_file -qsub_file qsub_file -force_pass ignore_qc_results 
...
if (!$vcf_obj->check_vcf(-vcf=>$vcf_file,-chromosome=>$chr)) {
	modules::Exception->throw("ERROR: $vcf_file fails for chr $chr\n") unless $OPT{force_pass};
}
