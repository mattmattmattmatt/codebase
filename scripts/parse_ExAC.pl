...
parse_ExAC.pl -input ExAC_file -outdir output_directory(default=pwd) [options]
...
parse_ExAC.pl -> Script to parse ExAC data
...
./parse_ExAC.pl -input ~/work/analysis/pipeline/annotations/exome_mcarthur/ExAC.r0.3.sites.vep.vcf -outdir /home/matt/work/pipeline/conf/human/GRCh37/exac/0.3
...
open(SNV,">$outdir/exac.snv") || die "Can't open file to write $outdir/snv\n";
open(DEL,">$outdir/exac.del") || die "Can't open file to write $outdir/del\n";
open(INS,">$outdir/exac.ins") || die "Can't open file to write $outdir/ins\n";
...
open(EXAC,"$exac") || die "Can't open file $exac\n";
...
while (<EXAC>) {
	...
	my ($var_count_str) = $rest =~ /AC_Adj=([0-9,]+);/;
	my ($allele_count) = $rest =~ /AN_Adj=([0-9]+);/;
	...
	for ( my $var_count = 0 ; $var_count < @vars ; $var_count++ ) {
		...
		push @{$coord_vars{$var_type}{$coord_key}}, join('~',$var_key,$var_count,$allele_count);
	}
	...
	for my $local_var_type (keys %coord_vars) {
		for my $coord_str (sort {my ($a_end) = $a =~ /(\d+)$/; my ($b_end) = $b =~ /(\d+)$/; $a_end <=> $b_end } keys %{$coord_vars{$local_var_type}}) {
			...
			for my $exac_entry (@{$coord_vars{$local_var_type}{$coord_str}}) {
				my ($var_key,$var_count,$allele_count) = split ('~',$exac_entry);
