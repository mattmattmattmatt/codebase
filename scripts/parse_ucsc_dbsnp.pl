    ...
    for my $variant (@alleles) {
       ...
       if ($variant =~ /^[ACGT]$/ && $refbase =~ /^[ACGT]$/) {
			...
			if ($freq_flag_local) {
       		...
       		#These cases are complicated so skip
       		if ($start == $end) {
				next;
       		}
			...
			my $new_start = my $new_end = $start + length($refbase);
			push @{$final_snps{$chr}{$new_start}{$new_end}{INS}{$new_variant}{rs}},$rs;

			if ($freq_flag_local) {
				$final_snps{$chr}{$new_start}{$new_end}{INS}{$new_variant}{varcount} = defined $line_allele_count{$variant}?$line_allele_count{$variant}:0;
				$final_snps{$chr}{$new_start}{$new_end}{INS}{$new_variant}{refcount} = defined $line_allele_count{$refbase}?$line_allele_count{$refbase}:0;
			} 
