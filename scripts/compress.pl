...
if ($OPT{dir}) {
	...
	if ($OPT{workdir}) {
		...
		if ($OPT{bam}) {
			...
			my @run_subdirs = split(",",$pipe_config->read('common','run_subdirs'));
	...
	my @new_files;
	for my $file (@files) {
		if ($compress eq 'uncompress' && $file !~ /$suffix$/) {
			#Here we need to add suffix
			push @new_files, $file . '.' . $suffix;
		} else {
			push @new_files, $file;
		}
	}
	
	$file_str = join(" ",@new_files);
...
for my $file ( @files ) {
	...
	if ($compress eq 'uncompress') {
		$file .= '.' . $suffix unless $file =~ /$suffix$/; #Add suffix unless already there
	}
