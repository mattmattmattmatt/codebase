...
my $bcftools_executable = 'bcftools';
my $vcf_out_base = $OPT{outvcf};
my $file_out = $OPT{outfile};
my $bam = $OPT{bam};
my @sv_types = qw(DEL DUP INV INS BND);
...
open(TRA,">$file_out.bnd") || modules::Exception->throw("Can't open file to write $file_out.tra"); 

my $fast = defined $OPT{fast}?1:0;

for my $sv_type ( @sv_types ) {
	(my $bcf_out = $vcf_out_base) =~ s/.vcf/_${sv_type}_somatic.bcf/;
	my $call_args = $pipe_conf->read($source_type,'binaries','delly','call','args_'.lc($sv_type));
	...
	if (-e $vcf_final && -s $vcf_final) {
		...
		#$delly->run(); #Run before hand now; see 
	}									
